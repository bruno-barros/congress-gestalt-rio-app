...
import AcoesAfirmativasBanner from "../components/ui/acoes-afirmativas-banner";
...
const Dashboard = (props: DashboardProps) => {
  ...
  function AcessarProdutos() {
    ...
    function handleClick() {
      setLoading(true);
      WpUser.saveRemoteSession(user.getId(), AuthToken.getToken()).then(
        (axios) => {
    ...
    return (
      <div
        className="border border-primary p-2 br-3"
        style={{ borderRadius: 5 }}
      >
        <div className="d-flex gap-3 align-items-center">
          Adquira itens extra do congresso
          <LoadingButton loading={loading} size="sm" onClick={handleClick}>
            Acessar produtos
          </LoadingButton>
        </div>
      </div>
        ...
        <div className="col-12 p-4">
          <div className="d-md-flex align-items-center justify-content-between mb-4">
            <h1 className="page-title">
              {t(user.canManageAbstracts() ? "eventos" : "meus-eventos")}
            </h1>
            {/* <AcessarProdutos /> */}
          </div>

          {/* {dump(current.getStartDate())} */}

          {current &&
            current.getEndDate() &&
            moment().isSameOrBefore(current.getEndDate(), "day") && (
              <AcoesAfirmativasBanner />
            )}
