...
function dataPatch(rows: any[]): ActivityUserSchema[] {
  ...
  return rows.map((row) => ({
    ...
    checkin_at: row.checkin_at
      ? new Date(row.checkin_at).toLocaleString()
      : null,
  }));
}
        ...
        Cell: ({ row }: { row: Row<ActivityUserSchema> }) => {
          ...
          return (
            <ButtonGroup size="sm">
              {checkin ? (
                <CancelCheckinModal
                  subscriptionId={Number(row.original.id)}
                  callable={<Button variant="danger">Cancelar</Button>}
                  onUpdate={onUpdate}
                />
              ) : (
                <DoCheckinModal
                  subscriptionId={Number(row.original.id)}
                  callable={<Button variant="success">Fazer Checkin</Button>}
                  onUpdate={onUpdate}
                />
              )}
            </ButtonGroup>
      ...
      <tbody {...getTableBodyProps()}>
        {rows.map((row: Row<ActivityUserSchema>) => {
          ...
          return (
            <tr
              {...row.getRowProps()}
              className={row.original?.checkin_at ? "bg-light-success" : ""}
            >
              {row.cells.map((cell) => (
