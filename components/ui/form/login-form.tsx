...
const LoginForm = () => {
  ...
  const LoginSchema = Yup.object().shape({
    ...
    password: Yup.string()
      .min(6, "validacao.curto")
      .required("validacao.obrigatorio"),
  });

  useEffect(() => {
    if (router.query?.redirect_to) {
      setRedirect(router.query.redirect_to);
    }
  }, [router.query]);
  ...
  async function handleSocialSuccess(user, provider) {
    ...
    if (!success) {
      ...
      data?.next_action === "profile_fase_1"
    ) {
      disp(
        setUpUser(
          {
  ...
  return (
    <div className="login-form">
      ...
      <PopOver
        title={t("cadastro.login-social")}
        text={t("cadastro.login-social-texto")}
        trigger={["hover", "focus"]}
        position="bottom"
      >
        <button
        ...
        {({ errors, touched, isValid, values }) => (
          <Form>
            ...
            <Text
              ...
              floatLabel
            />
            <LoadingButton disable={!isValid} loading={loading} block>
              {t("entrar")}
            </LoadingButton>
      ...
      />
      <PasswordRecover
        show={showPassRecover}
        onDismiss={(accountRecover) => {
          setShowPassRecover(false);
          if (accountRecover === true) setShowAccountRecover(true);
        }}
      />
      <AccountRecover
        show={showAccountRecover}
        onDismiss={() => setShowAccountRecover(false)}
      />
      <MaybeLoginWithEmail
        show={!!loginWithEmail}
        originalMessage={loginWithEmail}
        onDismiss={() => {
          setLoginWithEmail("");
        }}
      />
